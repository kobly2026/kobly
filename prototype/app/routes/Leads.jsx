...
// busca + drawer de detalhe (InfoLeads). window.KoblyLeads
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { DataTable, Badge, Input, IconButton, Icon, Button } = DS;
    ...
    return (
      <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 'var(--text-2xl)', fontWeight: 'var(--fw-bold)', color: 'var(--text-strong)', lineHeight: 1.1 }}>{window.KoblyApi.br(value)}</div>
  ...
  function LeadDrawer({ lead, onClose }) {
    ...
    const { Field, Drawer } = window;
    if (!lead) return null;
    const tagNames = (lead.tags || []).map((tid) => (DB.tags.find((t) => t.id === tid) || {}).nome).filter(Boolean);
    return (
      <Drawer open={!!lead} onClose={onClose} title={`${lead.nome} ${lead.sobrenome}`} subtitle={lead.email} width={440}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <Field label="Valor">{window.KoblyApi.money(lead.valorCompra)}</Field>
  ...
  function KoblyLeads() {
    ...
    const { useAsync, SkeletonRow } = window;
    const a = useAsync(() => window.KoblyApi.listLeads(store.session.empresaId || 'emp_1'), [store.role]);
            ...
            : (
              <DataTable
                ...
                columns={[
                  ...
                  { key: 'valorCompra', header: 'Valor', align: 'end', render: (r) => window.KoblyApi.money(r.valorCompra) },
