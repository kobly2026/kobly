...
// Construtor de fluxo. window.KoblyCampaigns
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { DataTable, Badge, Button, IconButton, Icon, TemplateCard, Input } = DS;
  ...
  function KoblyCampaigns() {
    ...
    const variant = window.useKoblyTweak('builderVariant', 'vertical');
    const empresaId = store.session.empresaId || 'emp_1';
    const a = useAsync(() => window.KoblyApi.listCampaigns(empresaId), [store.role]);
    ...
    async function openBuilder(id) {
      const c = await window.KoblyApi.getCampaign(id);
      setActive(c); setMode('builder');
    }
    async function create(tpl) {
      const c = await window.KoblyApi.createCampaign(tpl, empresaId);
      store.notify('success', 'Campanha criada');
      setActive(c); setMode('builder');
      a.reload();
    }
    async function setStatus(c, status) {
      await window.KoblyApi.setCampaignStatus(c.id, status);
      a.setData((d) => ({ ...d, campaigns: d.campaigns.map((x) => (x.id === c.id ? { ...x, status } : x)) }));
      store.notify('info', `"${c.nome}" → ${status}`);
    }

    if (mode === 'builder' && active) {
      return React.createElement(window.KoblyFlowBuilder, { campaign: active, variant, onBack: () => { setMode('list'); setActive(null); a.reload(); } });
    }
    if (mode === 'new') {
      return <NewCampaign templates={a.data ? a.data.templates : DB.templates} onPick={create} onCancel={() => setMode('list')} />;
    }
            ...
            : (
              <DataTable
                ...
                columns={[
                  ...
                  { key: 'abertura', header: 'Abertura', align: 'end', render: (r) => window.KoblyApi.pct(r.stats.taxaAbertura) },
                  { key: 'vendas', header: 'Recuperadas', align: 'end', render: (r) => <span style={{ color: r.stats.vendasRecuperadas ? 'var(--status-success-fg)' : 'var(--text-muted)', fontWeight: 'var(--fw-semibold)' }}>{window.KoblyApi.br(r.stats.vendasRecuperadas)}</span> },
  ...
  window.KoblyCampaigns = KoblyCampaigns;
})();
