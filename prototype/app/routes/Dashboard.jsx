...
// funil de entrega, feed ao vivo e visão consolidada (Gestor). window.KoblyDashboard
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { Card, Badge, Button, Select, Icon, Checklist, StatusLine } = DS;
  ...
  function KpiDrill({ metric, drill, range, onClose }) {
    const { Drawer, Chart } = window;
    if (!metric) return null;
    const d = drill[metric.key];
    const fmt = (v) => d && d.unit === 'pct' ? v + '%' : window.KoblyApi.br(v);
    return (
      ...
      <Card title="Disparos por gatilho" subtitle="Distribuição no período">
        <Chart
          ...
          options={{
            ...
            plotOptions: { pie: { donut: { size: '64%', labels: { show: true, total: { show: true, label: 'Total', color: C.textMuted, formatter: (w) => window.KoblyApi.br(w.globals.seriesTotals.reduce((a, b) => a + b, 0)) }, value: { color: C.textStrong, fontWeight: 700, formatter: (v) => window.KoblyApi.br(v) } } } } },
            tooltip: { y: { formatter: (v) => window.KoblyApi.br(v) } },
          }}
        />
      </Card>
          ...
          {funnel.map((f, i) => {
            ...
            return (
              <div key={f.etapa} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <span style={{ width: 78, flex: 'none', textAlign: 'end', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', color: 'var(--text-strong)' }}>{window.KoblyApi.br(f.valor)}</span>
  ...
  function KoblyDashboard() {
    ...
    const { useAsync, SkeletonDashboard, EmptyState } = window;
    const showOnboarding = window.useKoblyTweak('showOnboarding', true);
    const [range, setRange] = useState('30d');
    const [drill, setDrill] = useState(null);
    const a = useAsync(() => window.KoblyApi.getDashboard(store.role, store.session.empresaId, range), [store.role, store.session.empresaId, range]);
        ...
        {d.consolidated ? (
          <React.Fragment>
            ...
            <Card title="Contas gerenciadas" subtitle="Desempenho por conta de cliente" pad={false}>
              {React.createElement(window.KoblyAccountsTable, { rows: d.accounts })}
            </Card>
          </React.Fragment>
        ) : (
          <React.Fragment>
  ...
  Object.assign(window, { KoblyDashboard, KoblyAccountsTable });
})();
