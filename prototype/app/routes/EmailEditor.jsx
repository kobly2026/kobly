...
// preview, e envio de teste. Modal. window.KoblyEmailEditor
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { Button, IconButton, Input, Icon, StatusLine } = DS;
  ...
  function KoblyEmailEditor({ email, onClose, onSave }) {
    ...
    async function generate() {
      setAiBusy(true);
      const out = await window.KoblyAI.generateEmailHtml({ titulo: brief || assunto || titulo, cta: 'Concluir compra' });
      setHtml(out);
      setAiBusy(false);
      setTab('preview');
          ...
          <div style={{ flex: 1, minHeight: 0, display: 'grid', gridTemplateColumns: '340px 1fr' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: '1px solid var(--border-subtle)' }}>
                {window.Segmented && React.createElement(window.Segmented, { value: tab, onChange: setTab, options: [{ value: 'preview', label: 'Preview' }, { value: 'code', label: 'HTML' }] })}
                <Button variant="ghost" size="sm" iconLeft="send" style={{ marginInlineStart: 'auto' }} onClick={() => store.notify('success', 'E-mail de teste enviado')}>Enviar teste</Button>
              </div>
  ...
  window.KoblyEmailEditor = KoblyEmailEditor;
})();
