...
// Suporte responde; Cliente vê os próprios. window.KoblyTickets
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { Badge, Button, Avatar, Icon, IconButton } = DS;
  ...
  function KoblyTickets() {
    ...
    const a = useAsync(() => window.KoblyApi.listConversations(store.role, store.session.empresaId), [store.role]);
    ...
    async function send() {
      ...
      const updated = await window.KoblyApi.sendMessage(active.id, draft.trim(), autor, nome);
      setDraft('');
      a.setData((rows) => rows.map((c) => (c.id === updated.id ? updated : c)));
    }
    async function resolve() {
      if (!active) return;
      await window.KoblyApi.resolveConversation(active.id);
  ...
  window.KoblyTickets = KoblyTickets;
})();
