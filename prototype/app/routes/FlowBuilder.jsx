...
// window.KoblyFlowBuilder
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { Button, IconButton, Icon, Badge, Select, Input, StatusLine } = DS;
  ...
  function defaultConfig(tipo) {
    ...
    switch (tipo) {
      case 'Gatilho': return { tipoEvento: 'Abandono de carrinho', webhookId: DB.webhooks[0].id };
      case 'Adicionar Tag':
      case 'Remover Tag': return { tags: [] };
      case 'Envio de e-mail': return { emailId: DB.emails[0].id };
      case 'Acionar Fluxo': return { fluxoAlvo: '' };
      default: return {};
    }
  }
  function newStep(tipo) {
    return { id: 'st_' + Date.now() + '_' + Math.floor(Math.random() * 999), tipo, nome: tipo, atraso: tipo === 'Gatilho' ? 0 : 30, config: defaultConfig(tipo) };
  }
  ...
  function Palette({ onDragStart }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: 'var(--ls-wide)', fontWeight: 'var(--fw-semibold)', padding: '0 2px 4px' }}>Cards do fluxo</div>
        {CARD_TYPES.map((tipo) => (
          <div key={tipo} draggable onDragStart={(e) => onDragStart(e, 'new:' + tipo)}
            className="kbly-palette-card"
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 12px', background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', cursor: 'grab', transition: 'border-color var(--dur-fast), transform var(--dur-fast)' }}>
            ...
            <Icon name="grip-vertical" size={15} style={{ color: 'var(--text-subtle)', marginInlineStart: 'auto', flex: 'none' }} />
          </div>
        ))}
      </div>
    );
  ...
  function StepCard({ step, index, selected, onSelect, onDelete, onDragStart, compact }) {
    const DB = window.KoblyMockDB;
    ...
    const summary = (() => {
      ...
      if (step.tipo === 'Envio de e-mail') return (DB.emails.find((e) => e.id === c.emailId) || {}).titulo || 'e-mail';
      if (step.tipo === 'Acionar Fluxo') return (DB.campanhas.find((x) => x.id === c.fluxoAlvo) || {}).nome || 'selecionar fluxo';
      return '';
    })();
    return (
  ...
  // ----- Inspetor da etapa -----
  function Inspector({ step, onChange, onEditEmail }) {
    const DB = window.KoblyMockDB;
    if (!step) {
      return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {step.tipo === 'Gatilho' && (
          <React.Fragment>
            ...
            <Select label="Webhook de origem" value={c.webhookId} onChange={(e) => setCfg({ webhookId: e.target.value })} options={DB.webhooks.map((w) => ({ value: w.id, label: w.nome }))} />
          </React.Fragment>
        )}
        ...
        {step.tipo === 'Envio de e-mail' && (
          <React.Fragment>
            <Select label="E-mail" value={c.emailId} onChange={(e) => setCfg({ emailId: e.target.value })} options={DB.emails.map((em) => ({ value: em.id, label: em.titulo }))} />
        ...
        {step.tipo === 'Acionar Fluxo' && (
          <Select label="Fluxo a acionar" value={c.fluxoAlvo} onChange={(e) => setCfg({ fluxoAlvo: e.target.value })} options={[{ value: '', label: 'Selecionar…' }, ...DB.campanhas.map((x) => ({ value: x.id, label: x.nome }))]} />
        )}
  ...
  function KoblyFlowBuilder({ campaign, onBack, variant = 'vertical' }) {
    ...
    async function save() {
      await window.KoblyApi.saveFlow(campaign.id, steps, tagsMeta);
      setDirty(false);
      store.notify('success', 'Fluxo salvo');
    }
    async function toggleActive() {
      const next = status === 'Ativa' ? 'Pausada' : 'Ativa';
      await window.KoblyApi.setCampaignStatus(campaign.id, next);
      setStatus(next);
    ...
    steps.forEach((s, i) => {
      ...
      flowNodes.push(
        <div key={s.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: horizontal ? 0 : 0 }}>
          <StepCard step={s} index={i} selected={selId === s.id} onSelect={setSelId} onDelete={deleteStep} onDragStart={onDragStart} compact={compact} />
        </div>
      );
    });
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr 320px', gap: 16, alignItems: 'start' }}>
          ...
          {/* Canvas */}
          <div style={{ background: 'var(--surface-sunken)', border: '1px dashed var(--border-default)', borderRadius: 'var(--radius-md)', padding: horizontal ? '24px 16px' : '16px 24px', minHeight: 360, overflowX: horizontal ? 'auto' : 'visible' }}>
            {steps.length === 0 ? (
              <div style={{ height: 320, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, color: 'var(--text-muted)', textAlign: 'center' }}
                onDragOver={(e) => { e.preventDefault(); setOver(0); }} onDrop={(e) => { e.preventDefault(); onDrop(0); }}>
                <Icon name="git-branch" size={28} />
                <div style={{ fontSize: 'var(--text-sm)' }}>Arraste um card "Gatilho" para começar o fluxo.</div>
              </div>
            ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', padding: 18, minHeight: 200 }}>
              <Inspector step={selStep} onChange={updateStep} onEditEmail={(id) => setEmailModal(DB.emails.find((e) => e.id === id) || null)} />
            </div>
        ...
        {emailModal !== null && React.createElement(window.KoblyEmailEditor, { email: emailModal, onClose: () => setEmailModal(null), onSave: () => {} })}
      </div>
    );
  }
  window.KoblyFlowBuilder = KoblyFlowBuilder;
})();
