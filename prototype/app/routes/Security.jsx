...
// histórico de acesso, webhooks globais. window.KoblySecurity
(function () {
  const DS = window.KoblyDesignSystem_29b7f4;
  const { DataTable, Badge, Button, IconButton, Icon, Avatar } = DS;
  ...
  function KoblySecurity() {
    ...
    const a = useAsync(() => window.KoblyApi.getSecurity(), []);
    ...
    async function toggleUser(u) {
      ...
      await window.KoblyApi.setUserStatus(u.id, next);
    ...
    async function endSession(s) {
      await window.KoblyApi.endSession(s.id);
  ...
  window.KoblySecurity = KoblySecurity;
})();
